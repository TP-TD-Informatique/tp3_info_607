...
int main(int argc, char* argv[])
{
    ...
    int opt;
    while ((opt = getopt_long(argc, argv, "hl:i:K:Ise", long_options, NULL))
        != -1) {
        switch (opt) {
        case 'h':
            display_help(argv[0]);
    ...
    /* on créé un tube et on l'ouvre */
    unlink(fifo_name);
    if (-1 == mknod(fifo_name, S_IRUSR | S_IWUSR | S_IFIFO, 0)) {
        printf("Erreur lors de la création du tube %s.\n", fifo_name);
        printf("On abandonne tout.\n");
        exit(-1);
    };
    /* Attention, il faut ouvrir le tube en lecture/écriture pour conserver au
     * moins une connection "lecture" à tout moment. */
    fifo_wep = open(fifo_name, O_RDWR);
    if (-1 == fifo_wep) {
